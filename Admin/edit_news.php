...
if (isset($_GET['id'])) {
    ...
    $current_news = $result->fetch_assoc();
    if (!$current_news) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_news'])) {
    ...
    $sql = "UPDATE news SET headline = ?, date = ?, content = ?, elaborative_news = ? $image_sql WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssi", $headline, $date, $content, $elaborative_news, $news_id);
    $stmt->execute();
    header("Location: news.php");
}

// Fetch existing news for display
...
<div class="main-content">
    ...
    <div class="news-form animate-fade-in">
        ...
        <form method="POST" enctype="multipart/form-data">
            <input type="hidden" name="news_id" value="<?php echo $current_news['id']; ?>">
            <div class="form-group">
                <label>Headline</label>
                <input type="text" name="headline" class="form-control" required value="<?php echo htmlspecialchars($current_news['headline']); ?>">
            </div>


            <div class="form-group">
                <label>Date</label>
                <input type="date" name="date" class="form-control" required value="<?php echo $current_news['date']; ?>">
            </div>

            <div class="form-group">
                <label>Short Content</label>
                <textarea name="content" class="form-control" rows="3" required><?php echo htmlspecialchars($current_news['content']); ?></textarea>
            </div>

            <div class="form-group">
                <label>Elaborative News</label>
                <textarea name="elaborative_news" class="form-control" rows="5" required><?php echo htmlspecialchars($current_news['elaborative_news']); ?></textarea>
            </div>

            <button type="submit" name="update_news" class="btn">Update News</button>
            <button type="button" class="btn" onclick="location.href='news.php'">Cancel</button>
        </form>
    </div>
</div>
